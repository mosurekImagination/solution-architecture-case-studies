...
|Timeline |7 months, 3 phases
|Go-Live |April 2027
|Team |5 specialists (2 Data Eng + 1 Backend + 1 ML + 1 QA + 0.5 SA)
|Monthly Cost (post-launch) |$6K–$16K
|===
...
|Nightly delta + weekly full refresh
...
|Full MySQL → AWS RDS managed replication (WordPress)
|Replicates all 900GB to a managed RDS instance — including irrelevant tables. Cost: ~$600–800/mo. Not chosen: we extract only the 10–30% analytical subset via an on-prem read replica + Airflow DAGs, which costs nothing for the replica itself (client's own server) and avoids the 900GB replication overhead.
...
**7 questions — 4 blockers, 3 important.** We've minimized questions by making professional assumptions where possible (documented in ADR-002).
...
|Q0
|Provide a **random sample of 10K customer email addresses** for pre-engagement deliverability validation.
|Email is the primary revenue channel for the month 5 checkpoint. If >40% of addresses are invalid, we restructure the phase plan to prioritise mobile push instead. This 48-hour validation (<$100 cost) de-risks the entire engagement before contracts are signed.
...
|High
|High
|Direct SQL extraction + dbt tests detect breakage. Schema assumed stable (15-year system). If needed: fallback to RDS managed replication (+$600–800/mo OPEX, pushing total OPEX to $6.6K–$16.8K/month).
...
|Medium
|High
|Pre-engagement 10K-sample validation establishes baseline before contracts. Go/no-go gate: if <60% valid, restructure phases to elevate mobile push. Full validation in Phase 1 Week 2.
|===
...
|AWS infrastructure (ECS, S3, VPN, networking) |$15K (setup) |$1K–$3K
|AWS MWAA (Managed Airflow) |— |$300–$1.5K
|AWS SES (email delivery) |— |$100–$500
|Monitoring / Alerting |— |$500–$1K
|Development (team of 5, 7 months) |$411K |—
|Contingency (20%) |$82K |—
|*TOTAL CAPEX* |*~$493K* |
|*TOTAL OPEX* | |*$6K–$16K*
|===

_3-Year TCO: ~$695K–$1.07M (CAPEX + 3 years OPEX)_
...
|10 weeks (Oct–Dec 2026)
|WordPress extraction pipeline live, data in Snowflake, email validation running

|**Phase 2: Suggestion Engine & Email Channel**
|10 weeks (Dec 2026–Feb 2027)
|Customer segmentation, recommendation engine, first email campaign sent (month 5 revenue checkpoint)

|**Phase 3: Mobile & Wholesaler Expansion**
|10 weeks (Feb–Apr 2027)
|Mobile vendor integration, wholesaler data, push notifications, full platform go-live (April 2027)
|===
...
|1
|Provide 10K email sample for pre-engagement deliverability validation (Q0 above)
|Immediately (48-hour turnaround)
...
|3
|Answer discovery questions above (especially blockers Q1, Q4, Q6)
|Immediately
