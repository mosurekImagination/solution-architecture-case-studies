...
* Must operate within EU for GDPR data residency compliance — GDPR consent confirmed
* WordPress must remain as-is — 900GB heavily-customized MySQL, hosted on-prem
* Mobile app has APIs and own DB on vendor backend (2M installs) — lower integration risk than initially assumed
* **Budget: $500K** (confirmed) — fits Snowflake; eliminates Databricks full platform ($800K+)
...
* [x] Confirm AWS as cloud provider with client — ✅ **AWS confirmed** (hybrid with on-prem hosting); Snowflake on AWS EU region
* [ ] Provision Snowflake account in EU region (Frankfurt or Dublin)
* [ ] Set up Snowflake cost monitoring and usage alerts from day one — $500K budget requires strict cost discipline
...
* [ ] Run proof-of-concept: WordPress DB direct SQL read via VPN → S3 → Snowflake ingestion → basic segmentation query (see ADR-002)
...
* [ ] Schedule Snowflake + dbt training for client internal team — **critical: no data team exists, no hire plans**
