...
//
// MAINTENANCE:
//   This document should stay alive throughout the project. Assign an owner
//   (typically the Solution Architect) responsible for keeping it current.
//   A stale design doc is worse than no doc — it creates false confidence.
//   Update after every major decision, scope change, or architecture review.
//
// ============================================================================

== Version History

// Review cadence: update this table after every steering committee,
// major scope change, or architecture review. If the last entry is > 4 weeks
// old and the project is active, the document is going stale.
...
// The "one-screen" test: if a stakeholder only reads this box, they should
// understand what we're building, for whom, and why it matters.

****
*For* [target users] *who need* [core need], *the* [system name] *is a* [category]
*that* [key benefit]. *Unlike* [current alternative], *our solution* [key differentiator].
****
...
// Reversibility hint: "Easy" = config change or feature flag.
// "Hard" = significant rework but possible. "One-way" = migration or data loss if reversed.
// One-way decisions deserve an ADR and stakeholder sign-off.
...
|Choose the simplest solution that meets requirements today; refactor when proven necessary
|Avoids over-engineering and premature abstraction; reduces time-to-value
|Every "build for the future" decision must cite a concrete, funded requirement

|**Design for Changeability**
|Assume requirements and technology will change — architect for easy replacement of components
|Reduces cost of future change; aligns with agile/iterative delivery
|Prefer interfaces over implementations; isolate third-party dependencies behind adapters

|**YAGNI (You Aren't Gonna Need It)**
|Do not build capabilities until there is a validated need
|Reduces wasted effort, keeps codebase lean, shortens delivery
|Every "nice to have" feature must be justified by a user story with a real sponsor
...
.AS-IS Architecture
[d2,as-is-architecture,format=svg]
....
direction: right
...
[structurizr,c4-context,format=svg,view-key=ctx]
....
workspace {
...
[structurizr,c4-container,format=svg,view-key=cont]
....
workspace {
...
[structurizr,c4-deployment,format=svg,view-key=deploy]
....
workspace {
...
// Every architecture is a set of trade-offs. Make them explicit so
// stakeholders understand WHY, not just WHAT was chosen.
...
|[e.g., High availability during partitions]
|[e.g., Financial data requires ACID guarantees]

|[e.g., Build vs. Buy]
|[e.g., Buy (managed service)]
|[e.g., Build (custom solution)]
|[e.g., Team lacks ops capacity, faster time to market]

|[e.g., Flexibility vs. Simplicity]
|[e.g., Simpler monolith]
|[e.g., Flexible microservices]
|[e.g., Team of 4 cannot operate distributed system]
|===

=== Organizational Alignment (Conway's Law)

// "Any organization that designs a system will produce a design whose structure
// is a copy of the organization's communication structure." — Melvin Conway
//
// Map your service boundaries to team boundaries. If the architecture requires
// two teams to coordinate on every release, you've created a coupling point.

[cols="2,2,2"]
|===
|Component / Service |Owning Team |Communication Interface

|[e.g., Auth & User Management]
|[e.g., Platform Team]
|[e.g., OAuth2 tokens, user API]

|[e.g., Order Processing]
|[e.g., Commerce Team]
|[e.g., Event bus, REST API]

|[e.g., Mobile App]
|[e.g., Frontend Team]
|[e.g., Consumes API Gateway]
|===
...
[d2,component-architecture,format=svg,layout=elk]
....
direction: right
...
// ⚠️ Schema Evolution Warning: If multiple services share a database, every
// schema change requires coordinating all consumers. Prefer service-owned
// databases with published APIs. If shared DB is unavoidable, document the
// coupling and establish a schema change protocol.
...
[plantuml,flow-happy-path,format=svg]
....
@startuml
...
[plantuml,flow-error-handling,format=svg]
....
@startuml
skinparam backgroundColor white
skinparam sequenceMessageAlign center
...
// OPTIONS FRAMING GUIDANCE:
// Always present at least 3 options — this is a consulting best practice:
//   Option 1: MVP / lean (anchors "we can start small")
//   Option 2: Recommended / balanced (this is what you actually want them to pick)
//   Option 3: Premium / full (anchors the high end, makes Option 2 feel reasonable)
//
// The "decoy effect": Option 3 exists to make Option 2 look like the smart choice.
// If you only present 2 options, clients agonize. With 3, they feel in control.
...
|8 devs (3 BE + 2 FE + 1 QA + 1 DevOps + 1 SA)
|15 devs (multi-team)
...
// ============================================================================
// ESTIMATION APPROACH — READ BEFORE FILLING OUT COST SECTIONS
// ============================================================================
//
// Cost = Capacity × Time. Always estimate based on man-hours, not per feature.
//   Formula: (number of engineers) × (rate per month) × (duration) = CAPEX
//
// PRINCIPLES:
//   - Show rate × duration: clients buy engineering hours, make this transparent
//   - Use ranges, not point estimates: $[low]–$[high] acknowledges uncertainty
//   - Include a contingency line (20–30%): this covers unknowns, not profit
//   - Pair every cost line with a deliverable: show WHAT they get for the money
//   - Min 2 engineers per workstream: avoids bus factor, enables knowledge sharing
//   - Link phases to cost: each Gantt phase should have a corresponding cost row
//   - Present total AND breakdown: total for approval, breakdown for trust
//
// See also: template/life-lessons.md § Estimation & Costing
// ============================================================================
...
[plantuml,option-architecture,format=svg]
....
@startuml
skinparam backgroundColor white
...
// Principle: min 2 engineers per workstream (bus factor). Adding devs doesn't scale
// linearly — factor in communication overhead (Brooks's Law).

[cols="2,1,2"]
|===
|Workstream |Engineers |Roles

|[e.g., Backend]
|2
|[e.g., 1 Senior + 1 Mid-level]
...
// Cost = rate × time. Show the math — clients trust transparency.
// Use ranges ($[low]–$[high]) to acknowledge estimation uncertainty.
// Always include a contingency row — it covers unknowns, not profit margin.
...
|$[rate]/mo
|4 months
|[e.g., Core API, integrations, data model]
|$[low]–$[high]
...
|$[rate]/mo
|4 months
|[e.g., Mobile app, offline support]
|$[low]–$[high]
...
|—
|—
|_Covers: integration unknowns, scope clarification, vendor delays_
|*$[contingency]*
...
|_Spike traffic, additional environments_
|$[cost]
...
[plantuml,option-timeline,format=svg]
....
@startgantt
...
// Principle: link timeline to cost so stakeholders see what each phase costs.
// This answers "why does it cost this much?" alongside the Gantt's "why does it take this long?"

[cols="2,1,1,1,1"]
|===
|Phase |Duration |Team |Phase Cost |Cumulative
...
// Risk management tip: don't ignore medium-probability / medium-impact risks.
// They are the most dangerous because nobody owns them. Every risk needs an
// explicit owner. Include organizational and political risks — they kill
// more projects than technical risks do.

[cols="2,1,1,1,2,2"]
|===
|Risk |Category |Probability |Impact |Justification |Mitigation

|[Risk 1]
|Technical
|Medium
|High
|[Why this probability/impact — e.g., "No sandbox available for payment API"]
|[How to mitigate]
...
|Low
|Medium
|[e.g., "Key stakeholder on leave during UAT window"]
|[How to mitigate]
...
|High
|Medium
|[e.g., "Third-party API has no SLA guarantee"]
|[How to mitigate]
|===

_Categories: Technical, Organizational, Integration, Commercial, Regulatory, Resource_

=== Issues Log

// Issues are risks that have materialized. Track them here so they don't
// get lost in Slack threads and meeting minutes.

[cols="1,2,1,2,2,1"]
|===
|# |Issue |Severity |Impact |Resolution / Action |Owner

|1
|[e.g., Payment gateway sandbox not accessible]
|High
|[Blocks integration testing]
|[Escalated to client IT, fallback: mock service]
|[Name]

|2
|[e.g., Requirements for reporting module still undefined]
|Medium
|[Cannot estimate Phase 2 accurately]
|[Workshop scheduled for [date]]
|[Name]
|===
...
[cols="2,2,2,1,1"]
|===
|Dependency |Owner |Expected Date |Risk |Verified?
...
// ⚠️ "Verified?" column: a dependency isn't real until someone has tested it.
// "They said the API works" ≠ "We called the API and got a 200."
...
|Operability
|Team member leaves mid-sprint
|Remaining team can continue without blocking
|< 1 day onboarding for replacement on any component

|Evolvability
|New payment provider must be added
|Swap provider behind adapter interface
|< 2 sprint integration effort
|===

=== Scope Governance

// Scope creep is the #1 timeline killer. Establish the rules early.
// This section sets expectations for how changes are handled.
...
|All scope changes go through [change request board / product owner]. No verbal agreements.

|**Impact Assessment**
|Every change request gets a written impact note: timeline, cost, risk, trade-off.

|**Phase 2 Parking Lot**
|Features that don't make the cut go into a visible "Phase 2" backlog, not silent deletion.

|**Definition of Done**
|A feature is "done" when: code reviewed, unit tested (>80% coverage), integration tested, deployed to staging, documentation updated, PO accepted.
|===

=== Out of Scope

// ⚠️ The "Phase 2 Graveyard" warning: features parked here have a habit of
// never getting built. If something is critical, fight for it in Phase 1.
// If it's truly deferrable, be honest about it.
...
// This section answers the executive question: "Why should we spend this money?"
// Frame cost as investment, not expense. Show what the business GAINS.
...
|[e.g., 40 hrs/week × $50/hr = $104K/year]
|[e.g., 90% reduction → $94K/year savings]

|[e.g., Customer churn from slow system]
|[e.g., 5% monthly churn, ~$200K/year lost revenue]
|[e.g., Target < 2% churn with improved UX]

|[e.g., Compliance risk]
|[e.g., Potential $500K fine for non-compliance]
|[e.g., Automated controls eliminate manual audit gaps]
|===

**Payback period:** [e.g., Investment recovers in ~8 months based on operational savings]

**Strategic value (hard to quantify):** [e.g., Enables expansion to 3 new markets, positions for acquisition]

== Knowledge Transfer Plan

// Knowledge transfer is not optional — it's a deliverable. Plan it from day one,
// not as an afterthought in the last sprint. The project isn't done when the code
// is deployed; it's done when the client team can operate and extend it.
...
|Pair programming sessions (3 days)
|Client developers
|Last sprint
...
[plantuml,security-auth-flow,format=svg]
....
@startuml
skinparam backgroundColor white
skinparam sequenceMessageAlign center
...
// THE 30/70 RULE: Budget 30% of effort for "your" code, 70% for integrations,
// testing, and edge cases. Integrations always take longer than expected.
// Budget 3× your initial estimate for any third-party integration.

[cols="2,2,2,2,2"]
|===
|Integration |Pattern |Protocol |Error Handling |Failure Mode

|[Auth Provider]
|Request-Response (sync)
|OAuth2 / HTTPS
|Circuit breaker, 3 retries with exponential backoff
|[e.g., Degrade to cached tokens for 5 min]

|[Payment Gateway]
|Request-Response (sync)
|REST / HTTPS
|Idempotency keys, webhook confirmation
|[e.g., Queue payment, retry async, alert ops]

|[Notification Service]
|Event-Driven (async)
|Message queue (AMQP)
|Dead letter queue, 5 retries
|[e.g., Notifications delayed, not lost]

|[Analytics]
|Fire-and-Forget (async)
|Message queue
|Best effort, no retry
|[e.g., Data gap in analytics, acceptable]
|===

// Integration checkpoint: before each sprint that touches an integration,
// verify: sandbox accessible? credentials valid? rate limits sufficient?
// test data available? Don't discover issues on demo day.
...
[d2,infrastructure-topology,format=svg,layout=elk]
....
direction: right
...
[d2,observability-stack,format=svg,layout=elk]
....
direction: down
...
[plantuml,business-process-flow,format=svg]
....
@startuml
skinparam backgroundColor white
...
[plantuml,entity-lifecycle,format=svg]
....
@startuml
skinparam backgroundColor white
...
[plantuml,feature-mindmap,format=svg]
....
@startmindmap
...
[plantuml,work-breakdown,format=svg]
....
@startwbs
...
// ⚠️ Migration 3× Heuristic: whatever time you estimate for migration,
// multiply by 3. Data migration reveals data quality issues nobody knew about.
// Feature parity takes longer than new development because you're reverse-engineering.
...
// 1. What is cached? (query results, rendered pages, session data, config)
// 2. Where? (CDN edge, API gateway, application layer, database query cache)
// 3. TTL policy? (static: long TTL + cache-bust on deploy; dynamic: short TTL or event-based invalidation)
// 4. Invalidation strategy? (TTL expiry, event-driven purge, versioned keys)
// 5. Cache stampede protection? (lock/singleflight, stale-while-revalidate)
// 6. Cold start plan? (pre-warm on deploy, lazy load with fallback)
