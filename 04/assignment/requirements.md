...
The user shall be able to view a list of their active sessions, including browser/IP details, and log out selected sessions. Logging out from the **<u>current browser</u>** invalidates only the session for that browser. Other active sessions remain valid until the user explicitly logs them out from this screen or they expire naturally.
